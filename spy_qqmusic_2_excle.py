...
class Getsongs(Getqqmusic):
    ...
    def getlrc(self):
        for i in range(self.page):
            ...
            self.getjson(self.url,self.params)
    ...
    def getsong(self):
        ...
        sheet['F1'] = '链接'
        sheet['G1'] = 'MV链接'
        sheet['H1'] = datetime.datetime.now() 
        for i in range(self.page):
            self.params['p'] = str(i+1)
            self.getjson(self.url,self.params)
            for music in self.json_text["data"]["song"]["list"]:
                ...
                self.num += 1
    def menu(self):
        # self.singer = input('请输入要查询的歌手：')  #若要在类内输入，则init中应该不能放。
        # self.page = input('请输入要查询的页数：')
        # self.t = int(input('请输入要查询的类型（专辑: 8；MV :12；歌词:7；歌名:0）：'))
        if self.type_ == 7:
            self.getlrc()
        elif self.type_ == 12:
            self.getmv()
        elif self.type_ == 0:
            self.filename =  input('请输入要保存的文件名：')
            self.getsong()
class Getcomment(Getqqmusic):
    '''输入“歌手 歌名”,以空格隔开。
    ...
    def getid(self):
        ...
        self.getjson(url,params)
        self.topid = self.json_text['data']['song']['list'][0]['id']
    def getcomment(self):
        ...
        for i in range(self.page):
            ...
            self.getjson(url,params)   
            for comment in self.json_text['comment']['commentlist']:
